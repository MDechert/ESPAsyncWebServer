...
if [ ! -z "$TRAVIS_BUILD_DIR" ]; then
	...
	export GITHUB_REPOSITORY="MDechert/ESPAsyncWebServer"
fi
...
if [ "$BUILD_PIO" -eq 0 ]; then
	...
	if [[ "$TARGET_PLATFORM" == "esp32" ]]; then
		...
		git clone https://github.com/MDechert/AsyncTCP "$ARDUINO_USR_PATH/libraries/AsyncTCP" > /dev/null 2>&1
	...
	if [[ "$TARGET_PLATFORM" == "esp32" ]]; then
		BOARD="esp32dev"
		echo "Installing AsyncTCP ..."
		python -m platformio lib -g install https://github.com/MDechert/AsyncTCP.git > /dev/null 2>&1
		echo "BUILDING ESP32 EXAMPLES"
	else
		BOARD="esp12e"
		echo "Installing ESPAsyncTCP ..."
